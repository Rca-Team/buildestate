...
const DEFAULT_TITLE = 'RCA Estate - AI-Powered Luxury Real Estate | Find Your Dream Home';
...
export function useSEO({ title, description, image, url, type = 'website' }: SEOProps) {
  useEffect(() => {
    const fullTitle = title ? `${title} | RCA Estate` : DEFAULT_TITLE;
    ...
    return () => {
      ...
      setOG('og:title', 'RCA Estate - AI-Powered Luxury Real Estate');
      setOG('og:description', DEFAULT_DESCRIPTION);
      setOG('og:image', DEFAULT_IMAGE);
      setOG('og:url', SITE_URL);
      setOG('og:type', 'website');
      setTwitter('twitter:title', 'RCA Estate - AI-Powered Luxury Real Estate');
